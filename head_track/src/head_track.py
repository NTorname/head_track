...
class HeadTracker:
    def __init__(self):
        ...
        # marker_size = 0.065  # m
        self.marker_size = 0.02  # small markers
        # Aruco tag id's
        self.id_back = 380
        self.id_back_R = 403
        self.id_back_L = 473
    ...
    def callback(self, rawFrame):
        ...
        if ids is not None:
            ...
            q_rej_rate = 0.40
            tvec_rej_rate = 0.80
            e_list_x = self.reject_outliers(e_list_x, q_rej_rate)
            e_list_y = self.reject_outliers(e_list_y, q_rej_rate)
            e_list_z = self.reject_outliers(e_list_z, q_rej_rate)
            tvec_list_x = self.reject_outliers(tvec_list_x, tvec_rej_rate)
            tvec_list_y = self.reject_outliers(tvec_list_y, tvec_rej_rate)
            tvec_list_z = self.reject_outliers(tvec_list_z, tvec_rej_rate)
            ...
            self.pose_arr.append(copy.deepcopy(pose))
            self.pose_arr = self.pose_arr[1:11]
            ...
            x_filtered = self.reject_outliers(x_list, rej_out_rate_trans)
            y_filtered = self.reject_outliers(y_list, rej_out_rate_trans)
            z_filtered = self.reject_outliers(z_list, rej_out_rate_trans)
            rej_out_rate_orient = 0.75
            er_filtered = self.reject_outliers(er_list, rej_out_rate_orient)
            ep_filtered = self.reject_outliers(ep_list, rej_out_rate_orient)
            ey_filtered = self.reject_outliers(ey_list, rej_out_rate_orient)
    ...
    def reject_outliers(self, dataIn, factor=0.8):
...
# TODO
# OK here's the plan
# callback set a member pose
# in ros loop that is where we average and publish
def head_track():
    HT = HeadTracker()
